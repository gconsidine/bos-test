...
function setSources(cBot) {
    ...
    if (!Array.isArray(cBot.configs)) {
        throw new Error('cBot configuration is malformed. configs property must be an array');
    }
    ...
    return new Promise(function (resolve, reject) {
        ...
        for (var i = 0; i < cBot.configs.length; i++) {
            if (typeof cBot.configs[i] === 'string') {
                tmp = loadFromPath(path.resolve(cBot._cwd, cBot.configs[i]));
